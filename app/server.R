...
bulbar_symptoms = expand_names(sort(c("Speech", "Salivation", "Swallowing"), decreasing = TRUE))
motor_symptoms = expand_names(sort(c("Handwriting", "Cutting", "Dressing", "Turning", "Walking", "Climbing"), decreasing = TRUE))
respiratory_symptoms = expand_names(sort(c("Respiratory"), decreasing = TRUE))

symptom_list = list(Bulbar = bulbar_symptoms, Motor = motor_symptoms, Respiratory = respiratory_symptoms)

graph_input = graph_input %>% 
  mutate(
    symptom = expand_names(symptom),
    symptom_type = factor(
...
shinyServer(function(input, output, session) {
  ...
  output$instructions1 = renderText("This tab provides an overview of a given subject's predicted ALS progression. Switch to the \"Daily Tasks\" tab for more detail on their projected ability to perform individual tasks.")
  output$instructions1.5 = renderText("For each task, \"4\" represents full function and \"0\" represents complete loss of function")
  output$instructions2 = renderText("Move your mouse to a time in the future to see the predicted range of scores")
  
  output$About = renderText("Text about the app goes here")
})
